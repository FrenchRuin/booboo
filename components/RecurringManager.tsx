...
export default function RecurringManager({ currentUserId, onClose }: Props) {
      ...
      >
        ...
        <div className="px-5 pb-8 pt-2">
          ...
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-lg font-bold text-gray-900">고정비 관리</h2>
            <div className="flex items-center gap-1">
              <button
                onClick={() => { showAddForm ? resetForm() : setShowAddForm(true) }}
                className="text-xs text-blue-500 font-medium px-2.5 py-1.5 rounded-lg hover:bg-blue-50 transition-colors"
              >
                {showAddForm ? '취소' : '+ 추가'}
              </button>
                  ...
                  {formCategories.map((cat) => (
                    <button
                      ...
                      className={`flex flex-col items-center py-2 px-1 rounded-xl border transition-colors ${
                        formCategoryId === cat.id ? 'border-blue-400 bg-blue-50' : 'border-gray-100 bg-white'
                      }`}
                    >
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {items.map((item) => {
                ...
                return (
                  <div key={item.id} className="flex items-center gap-3 p-3.5 bg-gray-50 rounded-2xl">
