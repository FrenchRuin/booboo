    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-6" onClick={onCancel}>
      ...
      >
        <div className="px-6 pt-6 pb-5">
          <p className="text-sm text-gray-700 leading-relaxed text-center">{message}</p>
        </div>
        <div className={`flex border-t border-gray-100 ${cancelLabel ? '' : ''}`}>
          {cancelLabel && (
            <button
              onClick={onCancel}
          ...
          <button
            ...
            className={`flex-1 py-3.5 text-sm font-semibold transition-colors ${
              danger ? 'text-red-500 hover:bg-red-50' : 'text-blue-500 hover:bg-blue-50'
            }`}
          >
            {confirmLabel}
          </button>
